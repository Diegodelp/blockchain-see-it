...
function resolveRemoteBundleUrl() {
  const explicit = String(process.env.STREAMCHAIN_SELF_HOSTED_BUNDLE_URL || '').trim();
  if (explicit) {
    return explicit;
  }
...
async function maybeFetchRemoteBundle() {
  const url = resolveRemoteBundleUrl();
  if (!url) {
    return null;
  }

  const token = String(process.env.STREAMCHAIN_SELF_HOSTED_BUNDLE_TOKEN || '').trim();
...
export async function getSelfHostedBundle(rootDir) {
  const remoteBundle = await maybeFetchRemoteBundle();
  if (remoteBundle) {
    return remoteBundle;
  }
