...
import { getPublicNodeState, registerPeer, resolveFederationTrustedSignerPublicKey } from '@/lib/self-hosted-node';
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    trustedPeerSigners: state.trustedPeerSigners || [],
    chainConsensus: state.chainConsensus || null,
  });
}
...
export async function POST(request) {
  ...
  const rateLimitResponse = await assertRateLimit(request, { bucket: 'node-peers-post', limit: 20, windowMs: 60_000 });
  ...
  try {
    ...
    return NextResponse.json(await registerPeer(body));
  } catch (error) {
